...
-- 5. RLS on user_roles: users can see their own roles
CREATE POLICY "Users can view own roles"
  ON public.user_roles FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

-- 6. Admin can view all legal_requests
CREATE POLICY "Admins can view all requests"
  ON public.legal_requests FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- 7. Admin can update all legal_requests
CREATE POLICY "Admins can update all requests"
  ON public.legal_requests FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- 8. Add response columns to legal_requests
